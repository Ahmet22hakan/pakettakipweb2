    ...
    return Scaffold(
      ...
      appBar: AppBar(
        title: Row(
          children: [
            Text(
               container.read(languageProvider).isEnglish==null?"Kurye listesi":
                          ...
                          return Container(
                            ...
                            child: ListTile(
                              ...
                              subtitle: Row(
                                children: [
                                  Text(
                                     container.read(languageProvider).isEnglish==null?"Adreste": container.read(languageProvider).isEnglish!
                                          ? "At the Adress: ${context.read(infoProvider).adreste}"
                                          : "Unter der Adresse: ${context.read(infoProvider).adreste}",
                                      style: GoogleFonts.openSans()),
                                  const Spacer(),
                                  Text(
                                      container.read(languageProvider).isEnglish==null?"Kuryede":container.read(languageProvider).isEnglish!
                                          ? "On The Courier: ${context.read(infoProvider).kuryede}"
                                          : "Auf dem Fahrer: ${context.read(infoProvider).kuryede}",
                                      style: GoogleFonts.openSans()),
                                  Spacer(),
                                  Text(
                                    container.read(languageProvider).isEnglish==null?"Toplam: ${context.read(infoProvider).kuryede + context.read(infoProvider).adreste}":"Total: ${context.read(infoProvider).kuryede + context.read(infoProvider).adreste}",
                                    style: GoogleFonts.openSans(),
                                  ),
                                ],
                              ),
